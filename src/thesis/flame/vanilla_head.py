...
class FlameHeadVanilla(FlameHead):
    """
        ...
        with open(flame_model_path, "rb") as f:
            ...
            if sys.version_info.minor >= 9:
                ...
                def getargspec(func):
                    ...
                    return ArgSpec(args, varargs, varkw, tuple(defaults) if defaults else None)
    ...
    def add_teeth(self):
        ...
        vid_lip_outside_ring_upper = self.mask.get_vid_by_region(["lip_outside_ring_upper"],
                                                                 keep_order=True)

        vid_lip_outside_ring_lower = self.mask.get_vid_by_region(["lip_outside_ring_lower"],
                                                                 keep_order=True)
        ...
        # v = v[[0, 3, 1, 4, 3, 2, 6, 5]]
        v = v[[3, 2, 0, 1, 3, 4, 6,
               5]]  # todo: with this order, teeth_lower is not rendered correctly in the uv space
        uv = (torch.stack(torch.meshgrid(u, v, indexing="ij"),
                          dim=-1).permute(1, 0, 2).reshape(num_verts_teeth, 2))  # (#num_teeth, 2)
        ...
        posedirs = self.posedirs.reshape(len(self.parents) - 1, 9, num_verts_orig,
                                         3)  # (J*9, V*3) -> (J, 9, V, 3)
        posedirs = torch.cat(
            [posedirs, torch.zeros_like(posedirs[:, :, :num_verts_teeth])],
        ...
        self.J_regressor = torch.cat(
            [self.J_regressor,
             torch.zeros_like(self.J_regressor[:, :num_verts_teeth])],
            dim=1,
        ...
        self.lbs_weights = torch.cat(
            [self.lbs_weights,
             torch.zeros_like(self.lbs_weights[:num_verts_teeth])],
            dim=0,
...
class FlameMask(nn.Module):
    ...
    def create_custom_mask(self):
        ...
        self.v.register_buffer("left_eye", torch.cat([self.v.left_eye_region,
                                                      self.v.left_eyeball]))
        self.v.register_buffer("right_eye",
                               torch.cat([self.v.right_eye_region, self.v.right_eyeball]))
    ...
    def process_face_clusters(self, face_clusters):
        ...
        """
        fid2cid = torch.ones(
            self.num_faces + 1, dtype=torch.long)  # faces are always treated as foreground
