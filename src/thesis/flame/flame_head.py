"""
Flame head model, small changes to the original code, mainly added type hints and
type checking.

Assets can be downloaded from https://flame.is.tue.mpg.de/download.php
You should download FLAME 2023 (the version with jaw rotation)
"""

# Code heavily inspired by
# https://github.com/HavenFeng/photometric_optimization/blob/master/models/FLAME.py.
# Please consider citing their work if you find this code useful. The code is subject
# to the license available via
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        num_expr_params: int = 100,
        flame_model_path: str = FLAME_MODEL_PATH,
        flame_lmk_embedding_path: str = FLAME_LMK_PATH,
        flame_template_mesh_path: str = FLAME_MESH_PATH,
        flame_parts_path: str = FLAME_PARTS_PATH,
        include_mask: bool = True,
        add_teeth: bool = True,
    ) -> None:
        """
        Args:
            num_shape_params (int): Number of shape parameters. Defaults to 300.
            num_expr_params (int): Number of expression parameters. Defaults to 100.
            flame_model_path (str): Path to the FLAME model
            flame_lmk_embedding_path (str): Path to the FLAME landmark embedding.
            flame_template_mesh_path (str): Path to the FLAME template mesh.
            flame_parts_path (str): Path to the FLAME parts.
            include_mask (bool): Whether to include the mask.
            add_teeth (bool): Whether to add teeth to the model.
        """
        super().__init__()

        self.n_shape_params = num_shape_params
        self.n_expr_params = num_expr_params
        self.flame_parts_path = flame_parts_path

        with open(flame_model_path, "rb") as f:
            # check which python version is being used rn
            ...
            if sys.version_info.minor >= 9:
                ...
                def getargspec(func):
                    ...
                    This is a compatibility function that mimics the behavior of the
            ...
            # ss = dill.load(f, encoding="latin1")
        ...
        shapedirs = torch.cat(
            [
                shapedirs[:, :, :num_shape_params],
                shapedirs[:, :, 300 : 300 + num_expr_params],
            ],
            2,
        )
        ...
        if include_mask:
            self.mask = FlameMask(
                ...
                flame_parts_path=self.flame_parts_path,
            )
    ...
    def forward(
        ...
        rotation_euler = torch.zeros(batch_size, 3, device=shape.device)
        full_pose = torch.cat([rotation_euler, neck, jaw, eye], dim=1)
    ...
    def add_teeth(self):
        ...
        # upper, front
        v_teeth_upper_edge = (
            v_teeth_middle.clone() + torch.tensor([[0, mean_dist, 0]]) * 0.1
        )
        v_teeth_upper_root = (
            v_teeth_upper_edge + torch.tensor([[0, mean_dist, 0]]) * 2
        ...
        v_teeth_lower_edge -= (
            torch.tensor([[0, 0, mean_dist]]) * 0.4
        )  # slightly move the lower teeth to the back
        v_teeth_lower_root = (
            v_teeth_lower_edge - torch.tensor([[0, mean_dist, 0]]) * 2
        )  # scale the height of teeth
        ...
        uv = (
            torch.stack(torch.meshgrid(u, v, indexing="ij"), dim=-1)
            .permute(1, 0, 2)
            .reshape(num_verts_teeth, 2)
        )  # (#num_teeth, 2)
        ...
        self.shapedirs[
            vid_teeth_upper_root_back, :, : self.n_shape_params
        ] = shape_dirs_mean
        self.shapedirs[
            vid_teeth_upper_edge_back, :, : self.n_shape_params
        ] = shape_dirs_mean
        self.shapedirs[
            vid_teeth_lower_root_back, :, : self.n_shape_params
        ] = shape_dirs_mean
        self.shapedirs[
            vid_teeth_lower_edge_back, :, : self.n_shape_params
        ] = shape_dirs_mean

        # posedirs set to zero
        posedirs = self.posedirs.reshape(
            len(self.parents) - 1, 9, num_verts_orig, 3
        )  # (J*9, V*3) -> (J, 9, V, 3)
        posedirs = torch.cat(
        ...
        self.J_regressor = torch.cat(
            [self.J_regressor, torch.zeros_like(self.J_regressor[:, :num_verts_teeth])],
            dim=1,
        )  # (5, J) -> (5, J+num_verts_teeth)

        # lbs_weights manually set
        self.lbs_weights = torch.cat(
            [self.lbs_weights, torch.zeros_like(self.lbs_weights[:num_verts_teeth])],
            dim=0,
...
class FlameMask(nn.Module):
    ...
    def create_custom_mask(self):
        ...
        self.v.register_buffer(
            ...
            torch.tensor(
                [
                    ...
                    3344,
                    3350,
                    3362,  # 4-th ring from bottom (drop 7 front verts)
                ]
            ),
        )
        ...
        uniques, counts = torch.cat([self.v.scalp, face_and_neck]).unique(
            return_counts=True
        )
        intersection = uniques[counts == 2]
        uniques, counts = torch.cat([self.v.scalp, intersection]).unique(
            return_counts=True
        )
        ...
        self.v.register_buffer(
            "eyeballs", torch.cat([self.v.right_eyeball, self.v.left_eyeball])
        )
        self.v.register_buffer(
            "irises", torch.cat([self.v.right_iris, self.v.left_iris])
        )
        self.v.register_buffer(
            "left_eye", torch.cat([self.v.left_eye_region, self.v.left_eyeball])
        )
        self.v.register_buffer(
            "right_eye", torch.cat([self.v.right_eye_region, self.v.right_eyeball])
        )
        self.v.register_buffer(
            "eyelids", torch.cat([self.v.left_eyelid, self.v.right_eyelid])
        )
        self.v.register_buffer(
            "lip_inside_ring",
            torch.cat(
                [
                    self.v.lip_inside_ring_upper,
                    self.v.lip_inside_ring_lower,
                    torch.tensor([1594, 2730]),
                ]
            ),
        )

        # remove the intersection with irises from eyeballs and get the region for
        # scleras
        uniques, counts = torch.cat([self.v.eyeballs, self.v.irises]).unique(
            return_counts=True
        )
        intersection = uniques[counts == 2]
        uniques, counts = torch.cat([self.v.eyeballs, intersection]).unique(
            return_counts=True
        )
    ...
    def process_face_clusters(self, face_clusters):
        ...
        """
        fid2cid = torch.ones(
            self.num_faces + 1, dtype=torch.long
        ...
        for cid, cluster in enumerate(face_clusters):
            try:
                ...
                continue
            fid2cid[fids] = cid + 2
            # reserve cluster #0 for the background and #1 for faces that do not
