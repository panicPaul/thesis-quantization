...
def render_video_dynamic_gaussian_splatting(
    ...
    if smoothing_mode != 'none':
        raise NotImplementedError('Smoothing not yet implemented for dynamic gaussian splatting')
    ...
    n_render_frames = rigging_params.shape[0] - window_size + 1
    intrinsics = repeat(intrinsics, 'm n -> time m n', time=n_render_frames).cuda()
    if world_2_cam.shape == (4, 4):
        world_2_cam = repeat(world_2_cam, 'm n -> time m n', time=n_render_frames).cuda()
    else:
        world_2_cam = world_2_cam.cuda()
    se_3_transforms = UnbatchedSE3Transform(
        rotation=se_3_transforms.rotation.cuda()[window_size // 2:-window_size // 2 + 1],
        translation=se_3_transforms.translation.cuda()[window_size // 2:-window_size // 2 + 1],
    )
    rigging_params = rigging_params[window_size // 2:-window_size // 2 + 1].cuda()
