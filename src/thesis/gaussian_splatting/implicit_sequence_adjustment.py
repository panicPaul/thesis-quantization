...
class ImplicitSequenceAdjustment(nn.Module):
    ...
    def forward(
        self,
        sequence: Int[torch.Tensor, ""] | Int[torch.Tensor, "batch"],
        time_step: Int[torch.Tensor, ""] | Int[torch.Tensor, "batch"],
    ) -> Float[torch.Tensor, 'n_vertices 3'] | Float[torch.Tensor, 'batch n_vertices 3']:
        """
        Args:
        ...
        time_step = time_step.unsqueeze(-1) / 1_000
        ...
        x = nn.functional.tanh(x) * 1e-3  # hard clamp to avoid total collapse (total of 2e-3)
