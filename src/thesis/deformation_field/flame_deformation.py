...
class FlameDeformation(nn.Module):
    ...
    def __init__(
        ...
        self.motion_encoder = nn.Sequential(
            nn.Linear(3*window_size + 3, hidden_dim),  # also gets distance to vertex as input
            nn.SiLU(),
        ...
        if use_flame_vertex_latents:
            ...
            self.flame_vertex_encoder = nn.Sequential(
                nn.Linear(latent_dim*3 + 3, hidden_dim),  # also gets distance to vertex as input
                nn.SiLU(),
                nn.Linear(hidden_dim, hidden_dim),
                nn.SiLU(),
            )
    ...
    def forward(
        ...
        neighbor_motion_vectors = self.flame_knn.gather(
            nn_indices, vertex_motions)  # (n_gaussians, k, window_size*3)
        neighbor_positions = self.flame_knn.gather(nn_indices,
                                                   self.canonical_vertices)  # (n_gaussians, k, 3)
        neighbor_distances = torch.norm(
            means.unsqueeze(1) - neighbor_positions, dim=-1).unsqueeze(-1)  # (n_gaussians, k, 1)
        if self.use_flame_vertex_latents:
            neighbor_latents = self.embedding(nn_indices)

        # encode the motion vectors
        x = self.motion_encoder.forward(
            torch.cat((neighbor_motion_vectors, neighbor_distances), dim=-1))
        ...
        # flame vertex latents
        if self.use_flame_vertex_latents:
            vertex_latents = self.flame_vertex_encoder.forward(
                torch.cat((neighbor_latents, neighbor_distances), dim=-1))
            x = torch.cat((x, vertex_latents), dim=-1)

        # decode the motion vectors
        motion_vectors = self.motion_decoder.forward(x)
        means = means + motion_vectors[:, :3]
        quats_motions = nn.functional.normalize(motion_vectors[:, 3:], p=2, dim=-1)
        quats = nn.functional.normalize(quats, p=2, dim=-1)
        quats = quaternion_multiplication(quats_motions, quats)
